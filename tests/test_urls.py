...
def test_detects_internal_url(config: Config):
    ...
    assert len(urls) == 1
    assert "api.internal-domain.ch" in urls[0].description
...
def test_hostname_not_inside_url(config: Config):
    ...
    content = 'url = "https://srv-db01.internal-domain.ch/api"'
    findings = scan_content(content, "app.py", config)
    hostnames = [f for f in findings if f.finding_type == FindingType.INTERNAL_HOSTNAME]
...
def test_pom_dependency_tags_skipped(config: Config):
    ...
    assert any("git.internal-domain.ch" in t for t in url_texts)
    assert any("nexus.internal-domain.ch" in t for t in url_texts)
...
def test_pom_scm_urls_full_score(config: Config):
    ...
    content = (FIXTURES / "sample_pom.xml").read_text()
    findings = scan_content(content, "pom.xml", config)
    scm_findings = [f for f in findings if "git.internal-domain.ch" in f.description]
...
def test_pom_properties_url_medium_score(config: Config):
    ...
    content = (FIXTURES / "sample_pom.xml").read_text()
    findings = scan_content(content, "pom.xml", config)
    prop_findings = [f for f in findings if "app.internal-domain.ch" in f.description]
...
def test_properties_comment_skipped(config: Config):
    ...
    # db.internal-domain.ch from comment should NOT be found (M7 fix)
    # but db.internal-domain.ch from the actual jdbc url SHOULD be found
    # Actually the comment has "db.internal-domain.ch/admin" and the value has "srv-db01.internal-domain.ch"
    # The comment URL should not appear
    comment_url_found = any("db.internal-domain.ch/admin" in f.description for f in urls)
...
def test_properties_values_detected(config: Config):
    ...
    urls = [f for f in findings if f.finding_type == FindingType.INTERNAL_URL]
    assert any("app.internal-domain.ch" in f.description for f in urls)


def test_properties_full_score(config: Config):
    """Properties file findings should get full score."""
    content = 'app.url=https://api.internal-domain.ch/v1'
...
def test_sample_app_findings(config: Config):
    ...
    assert any("api.internal-domain.ch" in f.description for f in urls)
...
def test_snippet_has_context(config: Config):
    content = 'line1\nline2\nline3\nurl = "https://api.internal-domain.ch/v2"\nline5\nline6'
    findings = scan_content(content, "app.py", config)
